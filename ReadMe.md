...
- Dockerfile : image de conteneur basée sur CUDA, avec Python, GDAL, GeoPandas, PyTorch, SAM, Metashape et les dépendances géospatiales nécessaires.
...
- environment.yml : environnement Conda/Linux recommandé pour l’exécution locale.
- environment_windows.yml : environnement Conda/Windows adapté aux configurations de développement sur Windows.
- requirements.txt : dépendances Python minimales ou de référence pour l’installation traditionnelle.
...
- CUDA et un GPU compatible pour le modèle SAM;
...
### Option 1 — installation locale avec Conda
...
Si vous êtes sous Windows, vous pouvez utiliser :
...
Le projet est prêt pour une exécution dans Docker avec GPU :
