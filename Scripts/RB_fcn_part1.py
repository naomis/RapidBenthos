...
# %% RB_FilterSHP_clean
def Filter_segments(
    Segment_gpkg, output_segments_shp_path, output_pts_shp_path, output_pts_csv_path
):
    segments_df = gpd.read_file(Segment_gpkg)
    segments_df["area"] = segments_df.area  # add area
    segments_df_treshold_big = segments_df[
        (segments_df["area"] >= 0.075) & (segments_df["area"] < 2.5)
    ]  # filter larger shapes
    segment_df_treshold_small = segments_df[
        (segments_df["area"] >= 0.00005) & (segments_df["area"] < 0.075)
    ]  # filter small shapes
    ...
    for ind, row in tqdm(
        segment_small_filled.iterrows(), total=segment_small_filled.shape[0]
    ):
        rings = [i for i in row["geometry"].interiors]  # List all interior rings
        if len(rings) > 0:  # If there are any rings
            newgeom = None
            to_fill = [
                Polygon(ring) for ring in rings if Polygon(ring).area < sizelim
            ]  # List the ones to fill
    ...
    segment_df_all = segment_df_all[
        ...
        | (
            (segment_df_all["area"].between(0.0025, 0.005, inclusive="neither"))
            & (segment_df_all["pp"] >= 0.25)
        )
        | (
            (segment_df_all["area"].between(0.0005, 0.0025))
            & (segment_df_all["pp"] >= 0.3)
        )
    ]
    ...
    segments_df_filtered["center_point_x"] = segments_df_filtered.center_point.apply(
        lambda p: p.x
    )
    segments_df_filtered["center_point_y"] = segments_df_filtered.center_point.apply(
        lambda p: p.y
    )
...
class CameraStats:
    def __init__(self, camera):
        ...
        if chunk.camera_crs:
            transform = (
                Metashape.CoordinateSystem.datumTransform(crs, chunk.camera_crs)
                * transform
            )
        ...
        self.estimated_location = Metashape.CoordinateSystem.transform(
            location_ecef, ecef_crs, crs
        )
        if camera.reference.location:
            self.reference_location = camera.reference.location
            self.error_location = Metashape.CoordinateSystem.transform(
                self.estimated_location, crs, ecef_crs
            ) - Metashape.CoordinateSystem.transform(
                self.reference_location, crs, ecef_crs
            )
            self.error_location = (
                crs.localframe(location_ecef).rotation() * self.error_location
            )
        ...
        if camera.rotation_covariance:
            T = (
                localframe * camera_transform
            )  # to reflect rotation angles ypr (ecef_crs.localfram) or opk (crs.localframe)
            R0 = T.rotation()

            dR = antenna_transform.rotation()

            da = Metashape.utils.dmat2euler(
                R0 * dR, R0 * self.makeRotationDx(0) * dR, chunk.euler_angles
            )
            db = Metashape.utils.dmat2euler(
                R0 * dR, R0 * self.makeRotationDy(0) * dR, chunk.euler_angles
            )
            dc = Metashape.utils.dmat2euler(
                R0 * dR, R0 * self.makeRotationDz(0) * dR, chunk.euler_angles
            )
...
# v2
def camera_point_from_segment_centerPoint(
    MetashapeProject_path, Chunk_number, PhotoPath, OutputPath, CenterPoint_path
):
    ...
    if csv_is_utm and not chunk_is_utm:
        ...
        for _, row in RB_centroid.iterrows():
            pt = Metashape.Vector(
                (row.center_point_x, row.center_point_y, row.average_z)
            )
            pt_conv = Metashape.CoordinateSystem.transform(pt, utm_crs, crs)
            cx_list.append(pt_conv.x)
            cy_list.append(pt_conv.y)
        ...
        RB_centroid["center_point_y"] = cy_list
        print(
            f"  X : {RB_centroid.center_point_x.min():.6f} -> {RB_centroid.center_point_x.max():.6f}"
        )
        print(
            f"  Y : {RB_centroid.center_point_y.min():.6f} -> {RB_centroid.center_point_y.max():.6f}"
        )
    else:
    ...
    for index, row in tqdm(RB_centroid.iterrows(), total=RB_centroid.shape[0]):
        ...
        p = pd.DataFrame(
            {"x": [row.center_point_x], "y": [row.center_point_y], "z": [-5]}
        )
        p = np.asarray(p)
        ...
        for eps in [0.00001, 0.00002, 0.00005]:
            x_condition = np.where(
                np.logical_and(
                    v_Trans[:, 0] > p[0, 0] - eps, v_Trans[:, 0] < p[0, 0] + eps
                )
            )
            y_condition = np.where(
                np.logical_and(
                    v_Trans[:, 1] > p[0, 1] - eps, v_Trans[:, 1] < p[0, 1] + eps
                )
            )
            verts = v_Trans[np.intersect1d(x_condition, y_condition)]
            if len(verts) > 0:
        ...
        uv_10 = uv_bbox.sort_values(
            by=["dis_2D", "dis_3D"], ascending=[True, True]
        ).head(10)
...
def hexagrid(
    ...
    resolution,
    full_grid_path,
    segments_df_filtered,
    clip_grid_path,
    hexagrid_SAM_union_path_seg_shp,
    hexagrid_SAM_union_pst_shp,
    hexagrid_SAM_union_path_pts_csv,
):
    ...
    hexagrid_segments_df_filtered_difference = hexagrid.overlay(
        segments, how="difference"
    )
    hexagrid_segments_df_filtered_difference["segment_un"] = (
        hexagrid_segments_df_filtered_difference.index
    )
    hexagrid_segments_df_filtered_difference["segment_un"] = (
        "Hexa_" + hexagrid_segments_df_filtered_difference["segment_un"].astype(str)
    )
    hexagrid_segments_df_filtered_difference["area"] = (
        hexagrid_segments_df_filtered_difference.area
    )
    ...
    hexagrid_SAM = gpd.GeoDataFrame(
        pd.concat([segments2, hexagrid_segments_df_filtered_difference])
    )
