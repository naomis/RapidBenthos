...
MetashapeProject_path = (
    r"C:\Users\CMBU\Desktop\RapidBenthos_Data\M7_Reoriented\M7_reoriented_RB.psx"
)
Chunk_number = 0
...
print(
    f"  X range UTM : {RB_centroid.center_point_x.min():.1f} → {RB_centroid.center_point_x.max():.1f}"
)
print(
    f"  Y range UTM : {RB_centroid.center_point_y.min():.1f} → {RB_centroid.center_point_y.max():.1f}"
)
...
print(
    f"  X range WGS84 : {RB_centroid.cx_wgs.min():.6f} → {RB_centroid.cx_wgs.max():.6f}"
)
print(
    f"  Y range WGS84 : {RB_centroid.cy_wgs.min():.6f} → {RB_centroid.cy_wgs.max():.6f}"
)
...
for test_eps in [eps_auto, eps_auto * 2, eps_auto * 4, eps_auto * 8]:
    xi = np.where(
        np.logical_and(v_Trans[:, 0] > x0 - test_eps, v_Trans[:, 0] < x0 + test_eps)
    )
    yi = np.where(
        np.logical_and(v_Trans[:, 1] > y0 - test_eps, v_Trans[:, 1] < y0 + test_eps)
    )
...
for index, row in tqdm(RB_centroid.iterrows(), total=RB_centroid.shape[0]):
    ...
    for eps in [eps_auto, eps_auto * 2, eps_auto * 4, eps_auto * 8]:
        xi = np.where(
            np.logical_and(v_Trans[:, 0] > cx - eps, v_Trans[:, 0] < cx + eps)
        )
        yi = np.where(
            np.logical_and(v_Trans[:, 1] > cy - eps, v_Trans[:, 1] < cy + eps)
        )
    ...
    for camera in chunk.cameras:
        try:
            ...
            if (
                u < 0
                or u > camera.sensor.width
                or v_px < 0
                or v_px > camera.sensor.height
            ):
                continue
