...
def Select_class_ReefCloud_pts(
    RB_centroid_csv,
    RC_csv,
    label_file,
    polygon_file,
    ...
    df_classNum = df_all_sub_04.merge(
        label[["label_set", "Morphology", "ID_number", "Morpho_number"]],
        how="left",
        left_on="pred_desc",
        right_on="label_set",
    )
    ...
    Hexagrid_filter = df_classNum.loc[
        (
            (df_classNum.SAM_centroid.astype(str).str.contains("Hexa"))
            & (
                (
                    df_classNum["Morphology"].isin(
                        ["Soft_Coral", "Branching_non_acropora"]
                    )
                )
                | (
                    df_classNum["pred_desc"].isin(
                        ["ACO_MIL", "ACO_TEN", "ACO_OTH", "ACD", "AC_Bran_OTH", "ACT"]
                    )
                )
            )
        )
    ...
    df_classNum.drop(
        Hexagrid_filter.index, inplace=True
    )  # , 'Foliose', 'Branching_non_acropora',
    ...
    df_drop_morpho = pd.merge(
        df, most_occuring_morpho_df, how="inner", on="SAM_centroid"
    )

    df_drop_morpho = df_drop_morpho.dropna(subset=["Morphology_x"])
    df_t = df_drop_morpho.loc[
        df_drop_morpho.apply(lambda row: row.Morphology_x in row.Morphology_y, axis=1)
    ]
...
def format_percent_cover(label_polygon_seg, label_file):
    label = pd.read_csv(label_file)
    label["label_set"] = label["label_set"].replace({"AC _Bran_OTH": "AC_Bran_OTH"})
    RB_shp_df = geopandas.read_file(
        label_polygon_seg
    )  # RB_shp_df = geopandas.read_file(label_polygon_seg)
    RB_shp_df["pred_desc_"] = RB_shp_df.pred_desc_.replace({"G_STY ": "G_STY"})
    RB_shp_df_classes = RB_shp_df.groupby(["pred_desc_"])["area"].sum()
    RB_shp_df_classes_result = pd.DataFrame(
        columns=["SAM_10X4_Total_area", "SAM_10X4_Percent_cover"]
    )
    RB_shp_df_classes_result["SAM_10X4_Total_area"] = RB_shp_df_classes
    RB_shp_df_classes_result["SAM_10X4_Percent_cover"] = (
        RB_shp_df_classes / (RB_shp_df_classes.sum()) * 100
    )
    ...
    RB_shp_df_classes_result_group = RB_shp_df_classes_result_group.drop(
        ["pred_desc_"], axis=1
    )
    RB_shp_df_classes_result_group = RB_shp_df_classes_result_group.rename(
        columns={
            "SAM_10X4_Total_area": "Total_area",
            "SAM_10X4_Percent_cover": "Percent_cover",
        }
    )
    label = label.dropna()
    RB_shp_df_classes_result_group_morpho = label.merge(
        RB_shp_df_classes_result_group,
        left_on="label_set",
        right_on="class",
        how="outer",
    )
    RB_shp_df_classes_result_group_morpho = (
        RB_shp_df_classes_result_group_morpho.dropna(subset=["class"])
    )
    ...
    color = zip(
        RB_shp_df_classes_result_group_morpho["label_set"].unique(),
        RB_shp_df_classes_result_group_morpho["hex"].unique(),
    )
...
def stack_catplot(x, y, cat, stack, data, palette, out_fig, order):
    ...
    for i, c in enumerate(data[cat].unique()):
        ...
        for j, s in enumerate(data[stack].unique()):
            ...
            ax.bar(
                x=loc_x, height=height, bottom=bottom, width=width, zorder=10, color=s
            )
            ax2.bar(
                x=loc_x, height=height, bottom=bottom, width=width, zorder=10, color=s
            )
...
def ColonyLevel_Segments(label_polygon_seg, ColonyLevelSegments_shp):
    ...
    gdf_NH = gdf.loc[
        ~(
            (gdf.SAM_centro.astype(str).str.contains("Hexa"))
            & (~gdf["pred_desc_"].isin(["ACS", "ACX", "CB_OTH_C"]))
        )  #'G_POR'
        | (gdf["Morphology"].isin(["Acropora"]) & (gdf["area"] <= 0.0006))
    ]  # filterout Hexagrid
    gdf_buff = gdf_NH.loc[
        (gdf_NH["area"] <= 0.0025)
        | (
            ~gdf_NH["Morphology"].isin(
                ["Acropora", "Branching_non_acropora", "Soft_Coral"]
            )
        )
    ]
    ...
    # fill holes
    colony_gdf_c["rings"] = colony_gdf_c["geometry"].apply(
        lambda geom: [i for i in geom.interiors]
    )
    colony_gdf_c["to_fill"] = colony_gdf_c["rings"].apply(
        lambda rings: [Polygon(ring) for ring in rings if Polygon(ring).area < 0.001]
    )
    ...
    def process_row(chunk):  # Function to process each row
        ...
        return chunk.apply(
            lambda row: reduce(
                lambda geom1, geom2: geom1.union(geom2),
                [row["geometry"]] + row["to_fill"],
            ),
            axis=1,
        )
    ...
    chunks = [
        colony_gdf_c_toFill[i : i + chunk_size]
        for i in range(0, len(colony_gdf_c_toFill), chunk_size)
    ...
    colony_gdf_c_toFill["new_geometry"] = colony_gdf_c_toFill.apply(
        lambda row: reduce(
            lambda geom1, geom2: geom1.union(geom2), [row["geometry"]] + row["to_fill"]
        ),
        axis=1,
    )
    ...
    colony_gdf_c_merged = pd.concat(
        [colony_gdf_c_NotToFill, colony_gdf_c_toFill], sort=True
    )
